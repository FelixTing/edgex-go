...
func sendViaChannel(
	...
	lc.Debug("Sending notification: " + n.Id + ", via channel: " + c.String())
	var tr models.TransmissionRecord
	if c.Type == models.ChannelType(models.Email) {
		tr = sendMail(n.Content, c.EmailAddresses, n.ContentType, lc, config.Smtp)
	} else {
...
func resendViaChannel(
	...
	if t.Channel.Type == models.ChannelType(models.Email) {
		tr = sendMail(t.Notification.Content, t.Channel.EmailAddresses, t.Notification.ContentType, lc, config.Smtp)
	} else {
